...
export async function getInstallmentPlans(
  ...
  try {
    ...
    const { data: plans, error } = await withTenantFilter(query, tenantId);
...
export async function getInstallmentPlanById(
  ...
  try {
    ...
    const { data: plan, error } = await withTenantFilter(query, tenantId);
...
export async function deleteInstallmentPlan(
  ...
  try {
    ...
    const query = supabase.from("installment_plans").delete().eq("id", id);
 ...
 */
async function calculatePlanMetrics(
  planId: string,
  plan: Pick<
    InstallmentPlan,
    | "finance_amount"
  ...
  try {
    ...
    const totalPaid =
      plan.upfront_paid +
      paidInstallments.reduce((sum, i) => sum + i.amount_paid, 0);
    ...
    return {
      ...
      my_revenue:
        plan.business_model === "PRODUCT_OWNER" ? plan.upfront_paid : 0,
      total_interest: 0,
    };
  }
}
 ...
 */
async function generateInstallmentRecords(
  planId: string,
  plan: Pick<
    InstallmentPlanRecord,
    | "finance_amount"
